...
const getCategoryPillTitle = (grp: WarmupGroup, catName: string) => {
  if (isCategoryInWarmupGroup(grp, catName)) return 'Click to remove from this group'
  const other = getCategoryAssignedGroup(catName)
  if (other) return `Assigned to ${other.name}. Click to move to this group.`
  return 'Click to assign to this group'
}
